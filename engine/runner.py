...
def build_network(session):
    ...
    for current_set_name, pattern_set in all_data.items():
        ...
        for pattern in pattern_set['test']:
            neural.add_test(list(convert_pattern_split(pattern['data'])), get_output(current_set_id, num_sets))
        for pattern in pattern_set['train']:
            neural.add_train(list(convert_pattern_split(pattern['data'])), get_output(current_set_id, num_sets))
    return neural
